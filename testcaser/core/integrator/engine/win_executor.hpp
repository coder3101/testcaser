...
struct executor_engine {
  ...
  static testcaser::integrator::Result for_execution_of(
    ...
    FILE* fout = out != "N/A" ? fopen(out.c_str(), "w")
                              : NULL;
      _dup2(_fileno(fin), 0);
      if(fout != NULL){
    ...
    std::string command;
    if(is_python_script) command = "python3 "+bin;
    else command = bin;
    if(! CreateProcess(
      NULL,
    ...
    std::string kill_str = "taskkill /pid " + std::to_string(pi.dwProcessId) + " /f ";
    ...
    GetProcessMemoryInfo(pi.hProcess, reinterpret_cast<PPROCESS_MEMORY_COUNTERS>(&pmc), dummy);
    max_rss = pmc.WorkingSetSize / 1024;
    max_mem = pmc.PeakPagefileUsage / 1024;
    CloseHandle(pi.hProcess);
    if(ret_code != 0 && exit_stat == testcaser::integrator::ExitStatus::SUCCESS)
    ...
    return testcaser::integrator::Result(
      max_rss, max_mem, static_cast<size_t>((wll_time + SLACK_THRESHOLD)*1000000),
